...
class ReviewGameControllerTest extends AbstractControllerTest
{
    #[PU\Test]
    #[PU\DataProvider('indexValues')]
    public function indexDefault(string $storyClass, string $queryString, int $expectedNbGames): void
    {
        $storyClass::load();

        $user1 = UserFactory::repository()->findOneBy(['username' => 'user1']);
        $reviewRepo = ReviewFactory::repository();
        $reviews = $reviewRepo->findBy(['userId' => $user1->getId()]);
    ...
    #[PU\Test]
    #[PU\DataProvider('indexValues')]
    public function indexShowMore(string $storyClass, string $queryString, int $expectedNbGames): void
    {
        $storyClass::load();
    ...
    {
        return [
            'standard' => [ReviewIndexStandardStory::class, '', static::getContainer()->getParameter('app.default_limit')],
            'all' => [ReviewIndexStandardStory::class, 'limit=20', 20],
        ];
    }
