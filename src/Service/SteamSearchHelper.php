...
class SteamSearchHelper
{
    ...
    public function fetchSteamGame(int $id): ?array
    {
        try {
            ...
            $response = $this->httpClient->request('GET', 'https://store.steampowered.com/api/appdetails?appids=' . $id, [
    ...
    public function fillGame(Game $game, int $id, array $data): void
    {
        ...
        $game->setTypeGame(TypeGameEnum::fromSteam($data['type'] ?? ''));
        ...
        if ($releaseDate && preg_match('/(\d{4})/', $releaseDate, $matches)) {
            ...
            $this->exceptionManager->handle('warning', 'Unable to parse the release year or release date not found for the game with steamId: {steamId}', ['steamId' => $id]);
        }
        ...
        if (!empty($data['is_free'])) {
            ...
            $fullPrice = $data['price_overview']['initial'] ?? null;

            if ($currency === $this->currency && is_numeric($fullPrice) && $fullPrice > 0) {
                $game->setFullPrice((int) $fullPrice);
            } else {
                $game->setFullPrice(null);
                $this->exceptionManager->handle('warning', 'Unable to parse the price or invalid currency for the game with steamId: {steamId}', ['steamId' => $id]);
            }
        }

        // Managing developers : regroup all of them with commas, null if not present in the response
        $game->setDevelopers(!empty($data['developers']) ? implode(', ', $data['developers']) : null);
        ...
        $game->setGenres(!empty($data['genres']) ? implode(', ', array_column($data['genres'], 'description')) : null);

        // Managing description : null if not present in the response
        $game->setDescription($data['short_description'] ?? null);
